...
export default function Suggestion(props) {
  let { isBtn = null } = props;
  let btn = null;
  if (isBtn)
    btn = (
      <Button variant="primary" className="btn-sm" style={{ float: "right" }}>
        Follow
      </Button>
    );
  return (
    <ListGroup.Item className="suggestion-container pb-4">
      ...
      <div className="suggestion-content">
        <small>
          <strong style={{ fontSize: "13px" }}>prachi</strong>
        </small>
        <br />
        <small>HR || Open for collaboration</small>
      </div>
    </ListGroup.Item>
  );
}
